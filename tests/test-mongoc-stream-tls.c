...
test_mongoc_tls_no_verify (void)
{
   ...
   sopt.native_identity = "mongodb.com";
#else
...
#ifdef MONGOC_APPLE_NATIVE_TLS
   sopt.native_identity = "alt2.mongodb.com";
#else
...
#ifdef MONGOC_APPLE_NATIVE_TLS
   sopt.native_identity = "unicorn.wild.mongodb.com";
#else
   sopt.pem_file = PEMFILE_ALT;
   sopt.ca_file = CAFILE;
...
{
   bool run_tls_tests = true;

#ifdef MONGOC_APPLE_NATIVE_TLS
   if (! getenv("MONGOC_APPLE_NATIVE_TLS_TEST")) {
      run_tls_tests = false;
   }
#endif
